...
with open('api_input/query.txt', 'r') as file:
    query = file.read()

with open('api_input/context_window.txt', 'r') as file:
    context = file.read()

# Write the combined content to combined_context.txt
with open('api_input/combined_context.txt', 'w') as file:  # Use 'w' for writing
    file.write(query)
    file.write(context)

# The part below seems unnecessary if you're just creating combined_content.txt
with open('api_input/combined_content.txt', 'r') as file:
